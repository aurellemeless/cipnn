...
export class Phone {
	...
	findIprefix() {
		const iPrefix = CI_PREFIX.find((prfx) => this.phone.startsWith(prfx));
		return iPrefix ? iPrefix : null;
	}
	 ...
	 */
	findLocalPart() {
		return !!this.iPrefix ? this.phone.substring(this.iPrefix.length) : this.phone;
	}
	/**
	 *
	 ...
	 *
	 * @param {Boolean} iPrefix : Return with international prefix
	 * @returns {String | null}
	 */

	migrate8To10(iPrefix = true) {
		const phone8 = this.localPart;
		let phone10 = '';
		...
		return iPrefix && this.iPrefix ? this.iPrefix + phone10 : phone10;
	}
}
